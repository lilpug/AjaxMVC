...
(function ($) {
    ...
    function AttachModalDisplay(html, modalID, startDelay, stopDelay) {
        ...
        $(document).ajaxStart(function ()
        {
            ...
            //Checks if the modal is shown or if it was to quick to be shown
            if ($(modalID).hasClass('in'))
            {
                //starts the hide process with a delay which then deletes the element in a nice GUI way
                setTimeout(function ()
                {
                    ...
                    //Triggers the hide modal
                    $(modalID).modal("hide");
                }, stopDelay);
            }
            else //Not shown but the element does exist as its now appended in the start so remove it
            {
                //Removes the html
                $(modalID + "-container").remove();
            }
